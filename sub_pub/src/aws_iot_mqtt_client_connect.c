...
static uint32_t _aws_iot_get_connect_packet_length(IoT_Client_Connect_Params *pConnectParams) {
	...
	FUNC_ENTRY;
	qt_uart_dbg(uart1_conf.hdlr,"inside _aws_iot_get_connect_packet_length");
	...
	qt_uart_dbg(uart1_conf.hdlr,"exiting _aws_iot_get_connect_packet_length");
	FUNC_EXIT_RC(len);
}
...
static IoT_Error_t _aws_iot_mqtt_serialize_connect(unsigned char *pTxBuf, size_t txBufLen,
	...
	FUNC_ENTRY;
	qt_uart_dbg(uart1_conf.hdlr,"_aws_iot_mqtt_serialize_connect");

	if(NULL == pTxBuf || NULL == pConnectParams || NULL == pSerializedLen ||
	   (NULL == pConnectParams->pClientID && 0 != pConnectParams->clientIDLen) ||
	   (NULL != pConnectParams->pClientID && 0 == pConnectParams->clientIDLen)) {
		qt_uart_dbg(uart1_conf.hdlr,"NULL_VALUE_ERROR in _aws_iot_mqtt_serialize_connect");
	...
	switch(pConnectParams->MQTTVersion) {
		case MQTT_3_1_1:
		qt_uart_dbg(uart1_conf.hdlr,"Inside MQTT VERSION");
			break;
		default:
	...
	ptr = pTxBuf;
	qt_uart_dbg(uart1_conf.hdlr,"before _aws_iot_get_connect_packet_length");
	...
	qt_uart_dbg(uart1_conf.hdlr,"before aws_iot_mqtt_internal_init_header");
	...
	qt_uart_dbg(uart1_conf.hdlr,"aws_iot_mqtt_internal_write_char");
	...
	if(NULL == pConnectParams->pClientID) {
		qt_uart_dbg(uart1_conf.hdlr,"pConnectParams->pClientID : [%d]",pConnectParams->pClientID);
	...
	if(pConnectParams->isWillMsgPresent) {
		qt_uart_dbg(uart1_conf.hdlr,"message present [%d]",pConnectParams->isWillMsgPresent);
	...
	if(pConnectParams->pUsername) {
		qt_uart_dbg(uart1_conf.hdlr,"username [%s]",pConnectParams->pUsername);
		aws_iot_mqtt_internal_write_utf8_string(&ptr, pConnectParams->pUsername, pConnectParams->usernameLen);
	}

	if(pConnectParams->pPassword) {
		qt_uart_dbg(uart1_conf.hdlr,"password [%s]",pConnectParams->pPassword);
	...
	*pSerializedLen = (size_t) (ptr - pTxBuf);
	qt_uart_dbg(uart1_conf.hdlr,"_aws_iot_mqtt_serialize_connect end");
...
static IoT_Error_t _aws_iot_mqtt_deserialize_connack(unsigned char *pSessionPresent, IoT_Error_t *pConnackRc,
	...
	FUNC_ENTRY;
	qt_uart_dbg(uart1_conf.hdlr,"inside _aws_iot_mqtt_deserialize_connack");
	...
	qt_uart_dbg(uart1_conf.hdlr,"called aws_iot_mqtt_internal_read_char");
	if(CONNACK != MQTT_HEADER_FIELD_TYPE(header.byte)) {
		qt_uart_dbg(uart1_conf.hdlr,"aws_iot_mqtt_internal_read_char failed");
		FUNC_EXIT_RC(FAILURE);
	}

	/* read remaining length */
	rc = aws_iot_mqtt_internal_decode_remaining_length_from_buffer(curdata, &decodedLen, &readBytesLen);
	qt_uart_dbg(uart1_conf.hdlr,"aws_iot_mqtt_internal_decode_remaining_length_from_buffer returned [%d]",rc);
	if(SUCCESS != rc) {
		FUNC_EXIT_RC(rc);
	}
...
IoT_Error_t aws_iot_mqtt_connect(AWS_IoT_Client *pClient, IoT_Client_Connect_Params *pConnectParams) {
	...
	FUNC_ENTRY;
    qt_uart_dbg(uart1_conf.hdlr,"inside aws_iot_mqtt_connect");
	if(NULL == pClient) {
	...
	if(false == _aws_iot_mqtt_is_client_state_valid_for_connect(clientState)) {
		...
		qt_uart_dbg(uart1_conf.hdlr,"NETWORK_ALREADY_CONNECTED_ERROR");
	...
	qt_uart_dbg(uart1_conf.hdlr,"_aws_iot_mqtt_internal_connect returned [%d]",rc);

	if(SUCCESS != rc) {
		pClient->networkStack.disconnect(&(pClient->networkStack));
		qt_uart_dbg(uart1_conf.hdlr,"calling networkStack.disconnect");
		disconRc = pClient->networkStack.destroy(&(pClient->networkStack));
		qt_uart_dbg(uart1_conf.hdlr,"calling networkStack.destroy");
		if (SUCCESS != disconRc) {
			qt_uart_dbg(uart1_conf.hdlr,"NETWORK_DISCONNECTED_ERROR");
	...
	qt_uart_dbg(uart1_conf.hdlr,"pClient->clientStatus.clientStat %d\n", pClient->clientStatus.clientState);
	qt_uart_dbg(uart1_conf.hdlr,"exiting aws_iot_mqtt_connect");
	FUNC_EXIT_RC(rc);
}
...
static IoT_Error_t _aws_iot_mqtt_internal_disconnect(AWS_IoT_Client *pClient) {
	...
	qt_uart_dbg(uart1_conf.hdlr,"inside _aws_iot_mqtt_internal_disconnect");

	rc = aws_iot_mqtt_internal_serialize_zero(pClient->clientData.writeBuf, pClient->clientData.writeBufSize,
											  DISCONNECT,
											  &serialized_len);
	qt_uart_dbg(uart1_conf.hdlr,"aws_iot_mqtt_internal_serialize_zero returned [%d]",rc);
	if(SUCCESS != rc) {
		FUNC_EXIT_RC(rc);
	}
	...
	if(serialized_len > 0) {
		...
		qt_uart_dbg(uart1_conf.hdlr,"called aws_iot_mqtt_internal_send_packet");
	}
	...
	qt_uart_dbg(uart1_conf.hdlr,"networkStack.destroy");
	if(0 != rc) {
...
IoT_Error_t aws_iot_mqtt_disconnect(AWS_IoT_Client *pClient) {
	...
	FUNC_ENTRY;
	qt_uart_dbg(uart1_conf.hdlr,"inside aws_iot_mqtt_disconnect");
	if(NULL == pClient) {
		FUNC_EXIT_RC(NULL_VALUE_ERROR);
	}

	clientState = aws_iot_mqtt_get_client_state(pClient);
	qt_uart_dbg(uart1_conf.hdlr,"clientState is [%d]",clientState);
	...
	qt_uart_dbg(uart1_conf.hdlr,"aws_iot_mqtt_set_client_state returned [%d]",rc);
	if(SUCCESS != rc) {
		FUNC_EXIT_RC(rc);
	}

	rc = _aws_iot_mqtt_internal_disconnect(pClient);
	qt_uart_dbg(uart1_conf.hdlr,"_aws_iot_mqtt_internal_disconnect [%d]",rc);
	...
	qt_uart_dbg(uart1_conf.hdlr,"exiting aws_iot_mqtt_disconnect");
	FUNC_EXIT_RC(rc);
}
...
IoT_Error_t aws_iot_mqtt_attempt_reconnect(AWS_IoT_Client *pClient) {
	...
	FUNC_ENTRY;
	qt_uart_dbg(uart1_conf.hdlr,"inside aws_iot_mqtt_attempt_reconnect");
	if(NULL == pClient) {
	...
	if(aws_iot_mqtt_is_client_connected(pClient)) {
		qt_uart_dbg(uart1_conf.hdlr,"aws_iot_mqtt_is_client_connected -> CONNECTED");
		FUNC_EXIT_RC(NETWORK_ALREADY_CONNECTED_ERROR);
	}

	/* Ignoring return code. failures expected if network is disconnected */
	rc = aws_iot_mqtt_connect(pClient, NULL);
	qt_uart_dbg(uart1_conf.hdlr,"aws_iot_mqtt_connect returned [%d]",rc);
	...
	qt_uart_dbg(uart1_conf.hdlr,"aws_iot_mqtt_resubscribe has returned [%d]",rc);
	if(SUCCESS != rc) {
		FUNC_EXIT_RC(rc);
	}
	qt_uart_dbg(uart1_conf.hdlr,"exiting aws_iot_mqtt_attempt_reconnect");
