...
ClientState aws_iot_mqtt_get_client_state(AWS_IoT_Client *pClient) {
	FUNC_ENTRY;
	if(NULL == pClient) {
		qt_uart_dbg(uart1_conf.hdlr,"CLIENT_STATE_INVALID");
		return CLIENT_STATE_INVALID;
	}

	qt_uart_dbg(uart1_conf.hdlr,"exiting aws_iot_mqtt_get_client_state with [%d]",pClient->clientStatus.clientState);
...
IoT_Error_t aws_iot_mqtt_set_client_state(AWS_IoT_Client *pClient, ClientState expectedCurrentState,
	...
	IoT_Error_t threadRc = FAILURE;
#endif
    qt_uart_dbg(uart1_conf.hdlr,"inside aws_iot_mqtt_set_client_state");
	FUNC_ENTRY;
	if(NULL == pClient) {
		FUNC_EXIT_RC(NULL_VALUE_ERROR);
	}
...
IoT_Error_t aws_iot_mqtt_set_connect_params(AWS_IoT_Client *pClient, IoT_Client_Connect_Params *pNewConnectParams) {
	FUNC_ENTRY;
    qt_uart_dbg(uart1_conf.hdlr,"inside aws_iot_mqtt_set_connect_params");   
	if(NULL == pClient || NULL == pNewConnectParams) {
        qt_uart_dbg(uart1_conf.hdlr,"aws_iot_mqtt_set_connect_params are null ");
		FUNC_EXIT_RC(NULL_VALUE_ERROR);
	}
    ...
    qt_uart_dbg(uart1_conf.hdlr,"exiting aws_iot_mqtt_set_connect_params with SUCCESS");
	FUNC_EXIT_RC(SUCCESS);
}
...
IoT_Error_t aws_iot_mqtt_init(AWS_IoT_Client *pClient, IoT_Client_Init_Params *pInitParams) {
	...
	qt_uart_dbg(uart1_conf.hdlr,"para init in mqtt\n");

	FUNC_ENTRY;

	qt_uart_dbg(uart1_conf.hdlr,"inside mqtt init\n");
    if(NULL == pClient || NULL == pInitParams || 
		NULL == pInitParams->pHostURL || 0 == pInitParams->port ||
		NULL == pInitParams->pRootCALocation || 
		NULL == pInitParams->pDevicePrivateKeyLocation ||
		NULL == pInitParams->pDeviceCertLocation) {
        qt_uart_dbg(uart1_conf.hdlr,"iniside mqtt init : inside if");
		FUNC_EXIT_RC(NULL_VALUE_ERROR);
	}

	for(i = 0; i < AWS_IOT_MQTT_NUM_SUBSCRIBE_HANDLERS; ++i) {
        qt_uart_dbg(uart1_conf.hdlr,"iniside mqtt init : inside for [%d]",i );
	...
	//pClient->clientData.packetTimeoutMs = 0;//pInitParams->mqttPacketTimeout_ms;
	//pClient->clientData.commandTimeoutMs = 0;//pInitParams->mqttCommandTimeout_ms;
    ...
    qt_uart_dbg(uart1_conf.hdlr,"calling mqtt set connect parameters");
	rc = aws_iot_mqtt_set_connect_params(pClient, &default_options);
    qt_uart_dbg(uart1_conf.hdlr,"mqtt connect paramets has returned : %d ", rc);
	if(SUCCESS != rc) {
		FUNC_EXIT_RC(rc);
	}

#ifdef _ENABLE_THREAD_SUPPORT_
    qt_uart_dbg(uart1_conf.hdlr,"inside thread support ");
    ...
    qt_uart_dbg(uart1_conf.hdlr,"iot tls init");
    ...
    qt_uart_dbg(uart1_conf.hdlr, "Pingtimer initialized\n ");
...
bool aws_iot_is_autoreconnect_enabled(AWS_IoT_Client *pClient) {
	...
	if(NULL == pClient) {
		IOT_WARN(" Client is null! ");
        qt_uart_dbg(uart1_conf.hdlr,"client is null !!");
		FUNC_EXIT_RC(false);
	}
