...
static void _aws_iot_mqtt_force_client_disconnect(AWS_IoT_Client *pClient) {
	...
	qt_uart_dbg(uart1_conf.hdlr,"calling disconnect");
	pClient->networkStack.disconnect(&(pClient->networkStack));
	qt_uart_dbg(uart1_conf.hdlr,"calling destroy");
...
static IoT_Error_t _aws_iot_mqtt_handle_reconnect(AWS_IoT_Client *pClient) {
	...
	qt_uart_dbg(uart1_conf.hdlr,"inside _aws_iot_mqtt_handle_reconnect");
	if(!has_timer_expired(&(pClient->reconnectDelayTimer))) {
		qt_uart_dbg(uart1_conf.hdlr,"NETWORK_ATTEMPTING_RECONNECT");
	...
	qt_uart_dbg(uart1_conf.hdlr,"NETWORK_PHYSICAL_LAYER_DISCONNECTED");
	if(NULL != pClient->networkStack.isConnected) {
		rc = pClient->networkStack.isConnected(&(pClient->networkStack));
		qt_uart_dbg(uart1_conf.hdlr,"iot_tls_isconnected returned [%d]",rc);
	}

	if(NETWORK_PHYSICAL_LAYER_CONNECTED == rc) {
		qt_uart_dbg(uart1_conf.hdlr,"calling reconnect");
		...
		if(NETWORK_RECONNECTED == rc) {
			...
			qt_uart_dbg(uart1_conf.hdlr,"CLIENT_STATE_CONNECTED_YIELD_IN_PROGRESS");
			if(SUCCESS != rc) {
				FUNC_EXIT_RC(rc);
			}
			qt_uart_dbg(uart1_conf.hdlr,"NETWORK_RECONNECTED");
	...
	if(AWS_IOT_MQTT_MAX_RECONNECT_WAIT_INTERVAL < pClient->clientData.currentReconnectWaitInterval) {
		qt_uart_dbg(uart1_conf.hdlr,"NETWORK_RECONNECT_TIMED_OUT_ERROR");
...
static IoT_Error_t _aws_iot_mqtt_internal_yield(AWS_IoT_Client *pClient, uint32_t timeout_ms) {
	...
	//qt_uart_dbg(uart1_conf.hdlr,"inside _aws_iot_mqtt_internal_yield");
	...
	do {
		...
		if(CLIENT_STATE_PENDING_RECONNECT == clientState) {
			//qt_uart_dbg(uart1_conf.hdlr,"CLIENT_STATE_PENDING_RECONNECT");
			if(AWS_IOT_MQTT_MAX_RECONNECT_WAIT_INTERVAL < pClient->clientData.currentReconnectWaitInterval) {
				//qt_uart_dbg(uart1_conf.hdlr,"AWS_IOT_MQTT_MAX_RECONNECT_WAIT_INTERVAL");
				yieldRc = NETWORK_RECONNECT_TIMED_OUT_ERROR;
				//qt_uart_dbg(uart1_conf.hdlr,"_aws_iot_mqtt_internal_yield NETWORK_RECONNECT_TIMED_OUT_ERROR");
				break;
			}
			//qt_uart_dbg(uart1_conf.hdlr,"calling _aws_iot_mqtt_handle_reconnect");
			yieldRc = _aws_iot_mqtt_handle_reconnect(pClient);
			//qt_uart_dbg(uart1_conf.hdlr,"_aws_iot_mqtt_handle_reconnect [%d]",yieldRc);
			/* Network reconnect attempted, check if yield timer expired before
			 * doing anything else */
			continue;
		}

		//qt_uart_dbg(uart1_conf.hdlr,"calling internal_cycle_read in yield");
		yieldRc = aws_iot_mqtt_internal_cycle_read(pClient, &timer, &packet_type);
		if(SUCCESS == yieldRc) {
			qt_uart_dbg(uart1_conf.hdlr,"calling _aws_iot_mqtt_keep_alive");
			...
			if(NETWORK_SSL_READ_ERROR == yieldRc || NETWORK_SSL_WRITE_ERROR == yieldRc || NETWORK_SSL_WRITE_TIMEOUT_ERROR == yieldRc) {
				qt_uart_dbg(uart1_conf.hdlr,"calling _aws_iot_mqtt_handle_disconnect");
	...
	//qt_uart_dbg(uart1_conf.hdlr,"yieldrc [%d] before exit of _aws_iot_mqtt_internal_yield",yieldRc);
	FUNC_EXIT_RC(yieldRc);
}
...
IoT_Error_t aws_iot_mqtt_yield(AWS_IoT_Client *pClient, uint32_t timeout_ms) {
	...
	//qt_uart_dbg(uart1_conf.hdlr,"inside aws_iot_mqtt_yield");
	if(NULL == pClient || 0 == timeout_ms) {
		qt_uart_dbg(uart1_conf.hdlr,"aws_iot_mqtt_yield returned NULL_VALUE_ERROR");
		FUNC_EXIT_RC(NULL_VALUE_ERROR);
	}
	...
	if(CLIENT_STATE_DISCONNECTED_MANUALLY == clientState) {
		qt_uart_dbg(uart1_conf.hdlr,"aws_iot_mqtt_yield CLIENT_STATE_DISCONNECTED_MANUALLY");
	...
	if(CLIENT_STATE_PENDING_RECONNECT != clientState) {
		//qt_uart_dbg(uart1_conf.hdlr,"aws_iot_mqtt_yield CLIENT_STATE_PENDING_RECONNECT");
		/* Check if network is disconnected and auto-reconnect is not enabled */
		if(!aws_iot_mqtt_is_client_connected(pClient)) {
			qt_uart_dbg(uart1_conf.hdlr,"aws_iot_mqtt_yield NETWORK_DISCONNECTED_ERROR");
			FUNC_EXIT_RC(NETWORK_DISCONNECTED_ERROR);
		}
		...
		if(CLIENT_STATE_CONNECTED_IDLE != clientState) {
			qt_uart_dbg(uart1_conf.hdlr,"aws_iot_mqtt_yield MQTT_CLIENT_NOT_IDLE_ERROR");
			FUNC_EXIT_RC(MQTT_CLIENT_NOT_IDLE_ERROR);
		}

		rc = aws_iot_mqtt_set_client_state(pClient, CLIENT_STATE_CONNECTED_IDLE,
										   CLIENT_STATE_CONNECTED_YIELD_IN_PROGRESS);
		//qt_uart_dbg(uart1_conf.hdlr,"set client state to CLIENT_STATE_CONNECTED_YIELD_IN_PROGRESS");
		if(SUCCESS != rc) {
			FUNC_EXIT_RC(rc);
		}
	}

	//qt_uart_dbg(uart1_conf.hdlr,"calling _aws_iot_mqtt_internal_yield");
	...
	if(NETWORK_DISCONNECTED_ERROR != yieldRc && NETWORK_ATTEMPTING_RECONNECT != yieldRc) {
		...
		//qt_uart_dbg(uart1_conf.hdlr,"CLIENT_STATE_CONNECTED_IDLE");
		if(SUCCESS == yieldRc && SUCCESS != rc) {
			yieldRc = rc;
		}
	}

	//qt_uart_dbg(uart1_conf.hdlr,"yieldRc [%d] before exit of aws_iot_mqtt_yield",yieldRc);
	FUNC_EXIT_RC(yieldRc);
}
