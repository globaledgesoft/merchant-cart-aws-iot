...
static IoT_Error_t _aws_iot_mqtt_serialize_subscribe(unsigned char *pTxBuf, size_t txBufLen,
	...
	qt_uart_dbg(uart1_conf.hdlr,"inside _aws_iot_mqtt_serialize_subscribe");
	...
	*pSerializedLen = (uint32_t) (ptr - pTxBuf);
	...
	qt_uart_dbg(uart1_conf.hdlr,"exiting _aws_iot_mqtt_serialize_subscribe");
...
static IoT_Error_t _aws_iot_mqtt_internal_subscribe(AWS_IoT_Client *pClient, const char *pTopicName,
	...
	FUNC_ENTRY;
	qt_uart_dbg(uart1_conf.hdlr,"inside _aws_iot_mqtt_internal_subscribe");
	init_timer(&timer);
	...
	qt_uart_dbg(uart1_conf.hdlr,"calling _aws_iot_mqtt_serialize_subscribe");
	...
	qt_uart_dbg(uart1_conf.hdlr,"_aws_iot_mqtt_serialize_subscribe returned [%d]",rc);
	if(SUCCESS != rc) {
		FUNC_EXIT_RC(rc);
	}

	indexOfFreeMessageHandler = _aws_iot_mqtt_get_free_message_handler_index(pClient);
	qt_uart_dbg(uart1_conf.hdlr,"_aws_iot_mqtt_get_free_message_handler_index");
	...
	qt_uart_dbg(uart1_conf.hdlr,"aws_iot_mqtt_internal_send_packet returned [%d]",rc);
	if(SUCCESS != rc) {
		FUNC_EXIT_RC(rc);
	}

	/* wait for suback */
	qt_uart_dbg(uart1_conf.hdlr,"waiting for suback");
	rc = aws_iot_mqtt_internal_wait_for_read(pClient, SUBACK, &timer);
	qt_uart_dbg(uart1_conf.hdlr,"aws_iot_mqtt_internal_wait_for_read returned [%d]",rc);
	if(SUCCESS != rc) {
		FUNC_EXIT_RC(rc);
	}

	/* Granted QoS can be 0, 1 or 2 */
	qt_uart_dbg(uart1_conf.hdlr,"calling _aws_iot_mqtt_deserialize_suback");
	...
	qt_uart_dbg(uart1_conf.hdlr,"exiting _aws_iot_mqtt_internal_subscribe");
	FUNC_EXIT_RC(SUCCESS);
}
...
IoT_Error_t aws_iot_mqtt_subscribe(AWS_IoT_Client *pClient, const char *pTopicName, uint16_t topicNameLen,
	...
	FUNC_ENTRY;
	qt_uart_dbg(uart1_conf.hdlr,"inside aws_iot_mqtt_subscribe");
	...
	qt_uart_dbg(uart1_conf.hdlr,"clientState in aws_iot_mqtt_subscribe [%d]",clientState);
	...
	qt_uart_dbg(uart1_conf.hdlr,"calling _aws_iot_mqtt_internal_subscribe");
	subRc = _aws_iot_mqtt_internal_subscribe(pClient, pTopicName, topicNameLen, qos,
											 pApplicationHandler, pApplicationHandlerData);
	qt_uart_dbg(uart1_conf.hdlr,"_aws_iot_mqtt_internal_subscribe returned [%d]",subRc);
	...
	qt_uart_dbg(uart1_conf.hdlr,"exiting aws_iot_mqtt_subscribe");
	FUNC_EXIT_RC(subRc);
}
