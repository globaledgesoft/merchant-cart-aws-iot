...
static IoT_Error_t _aws_iot_mqtt_internal_serialize_publish(unsigned char *pTxBuf, size_t txBufLen, uint8_t dup,
	...
	FUNC_ENTRY;
	qt_uart_dbg(uart1_conf.hdlr,"inside _aws_iot_mqtt_internal_serialize_publish");
	if(NULL == pTxBuf || NULL == pPayload || NULL == pSerializedLen) {
		qt_uart_dbg(uart1_conf.hdlr,"_aws_iot_mqtt_internal_serialize_publish with NULL_VALUE_ERROR");
	...
	*pSerializedLen = (uint32_t) (ptr - pTxBuf);
	qt_uart_dbg(uart1_conf.hdlr,"exiting  _aws_iot_mqtt_internal_serialize_publish");
...
IoT_Error_t aws_iot_mqtt_internal_serialize_ack(unsigned char *pTxBuf, size_t txBufLen,
	...
	MQTTHeader header = {0};
	FUNC_ENTRY;
	qt_uart_dbg(uart1_conf.hdlr,"inside aws_iot_mqtt_internal_serialize_ack");
	if(NULL == pTxBuf || pSerializedLen == NULL) {
		qt_uart_dbg(uart1_conf.hdlr,"aws_iot_mqtt_internal_serialize_ack with NULL_VALUE_ERROR");
		FUNC_EXIT_RC(NULL_VALUE_ERROR);
	}
	...
	*pSerializedLen = (uint32_t) (ptr - pTxBuf);
	...
	qt_uart_dbg(uart1_conf.hdlr,"exiting aws_iot_mqtt_internal_serialize_ack");
	FUNC_EXIT_RC(SUCCESS);
}
...
static IoT_Error_t _aws_iot_mqtt_internal_publish(AWS_IoT_Client *pClient, const char *pTopicName,
	...
	qt_uart_dbg(uart1_conf.hdlr,"inside _aws_iot_mqtt_internal_publish");
	init_timer(&timer);
	...
	qt_uart_dbg(uart1_conf.hdlr,"calling _aws_iot_mqtt_internal_serialize_publish");
	...
	qt_uart_dbg(uart1_conf.hdlr,"_aws_iot_mqtt_internal_serialize_publish returned [%d]",rc);
	if(SUCCESS != rc) {
		FUNC_EXIT_RC(rc);
	}
	...
	if(QOS1 == pParams->qos) {
		qt_uart_dbg(uart1_conf.hdlr,"QOS1 wait for PUBACK");
		rc = aws_iot_mqtt_internal_wait_for_read(pClient, PUBACK, &timer);
		if(SUCCESS != rc) {
			FUNC_EXIT_RC(rc);
		}

		qt_uart_dbg(uart1_conf.hdlr,"calling aws_iot_mqtt_internal_deserialize_ack");
	...
	qt_uart_dbg(uart1_conf.hdlr,"exiting _aws_iot_mqtt_internal_publish");
...
IoT_Error_t aws_iot_mqtt_publish(AWS_IoT_Client *pClient, const char *pTopicName, uint16_t topicNameLen,
	...
	qt_uart_dbg(uart1_conf.hdlr,"inside aws_iot_mqtt_publish");
	if(NULL == pClient || NULL == pTopicName || 0 == topicNameLen || NULL == pParams) {
		qt_uart_dbg(uart1_conf.hdlr,"exiting aws_iot_mqtt_publish with NULL_VALUE_ERROR");
		FUNC_EXIT_RC(NULL_VALUE_ERROR);
	}
	...
	if(CLIENT_STATE_CONNECTED_IDLE != clientState && CLIENT_STATE_CONNECTED_WAIT_FOR_CB_RETURN != clientState) {
		qt_uart_dbg(uart1_conf.hdlr,"MQTT_CLIENT_NOT_IDLE_ERROR");
	...
	qt_uart_dbg(uart1_conf.hdlr,"calling _aws_iot_mqtt_internal_publish");
	pubRc = _aws_iot_mqtt_internal_publish(pClient, pTopicName, topicNameLen, pParams);
	qt_uart_dbg(uart1_conf.hdlr,"_aws_iot_mqtt_internal_publish returned [%d]",pubRc);
	...
	qt_uart_dbg(uart1_conf.hdlr,"exit aws_iot_mqtt_publish");
...
IoT_Error_t aws_iot_mqtt_internal_deserialize_publish(uint8_t *dup, QoS *qos,
	...
	if(NULL == dup || NULL == qos || NULL == retained || NULL == pPacketId) {
		qt_uart_dbg(uart1_conf.hdlr,"aws_iot_mqtt_internal_deserialize_publish with FAILURE");
	...
	if(4 > rxBufLen) {
		qt_uart_dbg(uart1_conf.hdlr,"MQTT_RX_BUFFER_TOO_SHORT_ERROR");
	...
	*payload = curData;
	...
	qt_uart_dbg(uart1_conf.hdlr,"exiting aws_iot_mqtt_internal_deserialize_publish");
	FUNC_EXIT_RC(SUCCESS);
}
...
IoT_Error_t aws_iot_mqtt_internal_deserialize_ack(unsigned char *pPacketType, unsigned char *dup,
	...
	FUNC_ENTRY;		
	qt_uart_dbg(uart1_conf.hdlr,"inside aws_iot_mqtt_internal_deserialize_ack");

	if(NULL == pPacketType || NULL == dup || NULL == pPacketId || NULL == pRxBuf) {
		qt_uart_dbg(uart1_conf.hdlr,"aws_iot_mqtt_internal_deserialize_ack with NULL_VALUE_ERROR");
		FUNC_EXIT_RC(NULL_VALUE_ERROR);
	}
	...
	*pPacketId = aws_iot_mqtt_internal_read_uint16_t(&curdata);
	...
	qt_uart_dbg(uart1_conf.hdlr,"exiting aws_iot_mqtt_internal_deserialize_ack");
	FUNC_EXIT_RC(SUCCESS);
}
