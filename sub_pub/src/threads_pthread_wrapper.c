...
//	if(0 != pthread_mutex_init(&(pMutex->lock), NULL)) {
    if(0 != tx_mutex_create(pMutex->lock, "pMutex->lock", TX_NO_INHERIT)){
...
//int rc = pthread_mutex_lock(&(pMutex->lock));
  int rc = tx_mutex_get(pMutex->lock, TX_WAIT_FOREVER)
	if(0 != rc) {
...
//int rc = pthread_mutex_trylock(&(pMutex->lock));
  int rc = tx_mutex_get(pMutex->lock, TX_NO_WAIT);
	if(0 != rc) {
		return MUTEX_LOCK_ERROR;
	}
...
//	if(0 != pthread_mutex_destroy(&(pMutex->lock))) {
    if(0 != tx_mutex_delete(pMutex->lock)) {
