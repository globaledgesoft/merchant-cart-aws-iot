...
size_t aws_iot_mqtt_internal_write_len_to_buffer(unsigned char *buf, uint32_t length) {
	...
	qt_uart_dbg(uart1_conf.hdlr,"inside aws_iot_mqtt_internal_write_len_to_buffer");
	FUNC_ENTRY;
	do {
	...
	qt_uart_dbg(uart1_conf.hdlr,"exiting aws_iot_mqtt_internal_write_len_to_buffer");
	FUNC_EXIT_RC(outLen);
}
...
IoT_Error_t aws_iot_mqtt_internal_decode_remaining_length_from_buffer(unsigned char *buf, uint32_t *decodedLen,
	...
	qt_uart_dbg(uart1_conf.hdlr,"inside aws_iot_mqtt_internal_decode_remaining_length_from_buffer");
	multiplier = 1;
	len = 0;
	*decodedLen = 0;
	...
	*readBytesLen = len;
	...
	qt_uart_dbg(uart1_conf.hdlr,"exiting aws_iot_mqtt_internal_decode_remaining_length_from_buffer");
	FUNC_EXIT_RC(SUCCESS);
}

uint32_t aws_iot_mqtt_internal_get_final_packet_length_from_remaining_length(uint32_t rem_len) {

	qt_uart_dbg(uart1_conf.hdlr,"inside aws_iot_mqtt_internal_get_final_packet_length_from_remaining_length");
	...
	qt_uart_dbg(uart1_conf.hdlr,"exiting aws_iot_mqtt_internal_get_final_packet_length_from_remaining_length");
	return rem_len;
}
...
uint16_t aws_iot_mqtt_internal_read_uint16_t(unsigned char **pptr) {
	...
	*pptr += 2;
	qt_uart_dbg(uart1_conf.hdlr,"inside aws_iot_mqtt_internal_read_uint16_t");
	qt_uart_dbg(uart1_conf.hdlr,"exiting aws_iot_mqtt_internal_read_uint16_t");
	return len;
}
...
void aws_iot_mqtt_internal_write_uint_16(unsigned char **pptr, uint16_t anInt) {
	qt_uart_dbg(uart1_conf.hdlr,"inside aws_iot_mqtt_internal_write_uint_16");
	**pptr = (unsigned char) (anInt / 256);
	(*pptr)++;
	**pptr = (unsigned char) (anInt % 256);
	(*pptr)++;
	qt_uart_dbg(uart1_conf.hdlr,"exiting aws_iot_mqtt_internal_write_uint_16");
}
...
IoT_Error_t aws_iot_mqtt_internal_init_header(MQTTHeader *pHeader, MessageTypes message_type,
	...
	uint8_t type = 0;
	FUNC_ENTRY;
	qt_uart_dbg(uart1_conf.hdlr,"inside aws_iot_mqtt_internal_init_header");
	if(NULL == pHeader) {
	...
	switch(message_type) {
		case UNKNOWN:
			qt_uart_dbg(uart1_conf.hdlr,"UNKNOWN:");
			/* Should never happen */
			return FAILURE;
		case CONNECT:
			qt_uart_dbg(uart1_conf.hdlr,"CONNECT:");
			type = 0x01;
			break;
		case CONNACK:
			qt_uart_dbg(uart1_conf.hdlr,"CONNACK:");
			type = 0x02;
			break;
		case PUBLISH:
			qt_uart_dbg(uart1_conf.hdlr,"PUBLISH:");
			type = 0x03;
			break;
		case PUBACK:
			qt_uart_dbg(uart1_conf.hdlr,"PUBACK:");
			type = 0x04;
			break;
		case PUBREC:
			qt_uart_dbg(uart1_conf.hdlr,"PUBREC:");
			type = 0x05;
			break;
		case PUBREL:
			qt_uart_dbg(uart1_conf.hdlr,"PUBREL:");
			type = 0x06;
			break;
		case PUBCOMP:
			qt_uart_dbg(uart1_conf.hdlr,"PUBCOMP:");
			type = 0x07;
			break;
		case SUBSCRIBE:
			qt_uart_dbg(uart1_conf.hdlr,"SUBSCRIBE:");
			type = 0x08;
			break;
		case SUBACK:
			qt_uart_dbg(uart1_conf.hdlr,"SUBACK:");
			type = 0x09;
			break;
		case UNSUBSCRIBE:
			qt_uart_dbg(uart1_conf.hdlr,"UNSUBSCRIBE:");
			type = 0x0A;
			break;
		case UNSUBACK:
			qt_uart_dbg(uart1_conf.hdlr,"UNSUBACK:");
			type = 0x0B;
			break;
		case PINGREQ:
			qt_uart_dbg(uart1_conf.hdlr,"PINGREQ:");
			type = 0x0C;
			break;
		case PINGRESP:
			qt_uart_dbg(uart1_conf.hdlr,"PINGRESP");
			type = 0x0D;
			break;
		case DISCONNECT:
			qt_uart_dbg(uart1_conf.hdlr,"DISCONNECT:");
			type = 0x0E;
			break;
		default:
			qt_uart_dbg(uart1_conf.hdlr,"Should never happen");
			/* Should never happen */
	...
	qt_uart_dbg(uart1_conf.hdlr,"exiting aws_iot_mqtt_internal_init_header");
	FUNC_EXIT_RC(SUCCESS);
}
...
IoT_Error_t aws_iot_mqtt_internal_send_packet(AWS_IoT_Client *pClient, size_t length, Timer *pTimer) {
	...
	FUNC_ENTRY;
	qt_uart_dbg(uart1_conf.hdlr,"inside aws_iot_mqtt_internal_send_packet");
	if(NULL == pClient || NULL == pTimer) {
		qt_uart_dbg(uart1_conf.hdlr,"NULL_VALUE_ERROR");
		FUNC_EXIT_RC(NULL_VALUE_ERROR);
	}

	if(length >= pClient->clientData.writeBufSize) {
		qt_uart_dbg(uart1_conf.hdlr,"MQTT_TX_BUFFER_TOO_SHORT_ERROR");
	...
	qt_uart_dbg(uart1_conf.hdlr,"sockfd in read [%d]",pClient->networkStack.tlsDataParams.sock_fd);
	...
	while(sent < length && !has_timer_expired(pTimer)) {
		rc = pClient->networkStack.write(&(pClient->networkStack),
						 ...
						 pTimer,
						 &sentLen);
		qt_uart_dbg(uart1_conf.hdlr,"sent message : [%c]",pClient->clientData.writeBuf[sent]);
		if(SUCCESS != rc) {
			/* there was an error writing the data */
		break;
		qt_uart_dbg(uart1_conf.hdlr,"there was an error writing the data");
		}
		sent += sentLen;
	}
	...
	if(sent == length) {
		...
		qt_uart_dbg(uart1_conf.hdlr,"we have sent the packet successfully");
		//countdown_sec(&c->pingTimer, c->clientData.keepAliveInterval);
		FUNC_EXIT_RC(SUCCESS);
	}

	qt_uart_dbg(uart1_conf.hdlr,"exiting aws_iot_mqtt_internal_send_packet");
	FUNC_EXIT_RC(rc) 
}
...
static IoT_Error_t _aws_iot_mqtt_internal_readWrapper( AWS_IoT_Client *pClient, size_t offset, size_t size, Timer *pTimer, size_t * read_len ) {
	...
	qt_uart_dbg(uart1_conf.hdlr,"inside _aws_iot_mqtt_internal_readWrapper");

	qt_uart_dbg(uart1_conf.hdlr,"sockfd in read [%d]",pClient->networkStack.tlsDataParams.sock_fd);

    byteToRead = ( offset + size ) - pClient->clientData.readBufIndex;
	qt_uart_dbg(uart1_conf.hdlr,"before if byteToRead : [%d]",byteToRead);
    ...
    if ( byteToRead > 0 )
    {
        ...
        qt_uart_dbg(uart1_conf.hdlr,"calling networkStack.read and returned [%d]",rc);
        byteToRead = ( offset + size ) - ((int)pClient->clientData.readBufIndex);
        *read_len = size - (size_t)byteToRead;
		qt_uart_dbg(uart1_conf.hdlr,"in if byteToRead : [%d]",byteToRead);
    }
    else
    {
        *read_len = size;
		qt_uart_dbg(uart1_conf.hdlr,"read_len : [%d]",size);
		qt_uart_dbg(uart1_conf.hdlr,"exiting  networkStack.read with SUCCESS [%d]",rc);
        rc = SUCCESS;
    }
...
static IoT_Error_t _aws_iot_mqtt_internal_read_packet(AWS_IoT_Client *pClient, Timer *pTimer, uint8_t *pPacketType) {
	...
	qt_uart_dbg(uart1_conf.hdlr,"coundown %d",pClient->clientData.packetTimeoutMs);
	...
	if(NETWORK_SSL_NOTHING_TO_READ == rc) {
		qt_uart_dbg(uart1_conf.hdlr,"MQTT_NOTHING_TO_READ");
	...
	qt_uart_dbg(uart1_conf.hdlr,"_aws_iot_mqtt_internal_decode_packet_remaining_len returned [%d]",rc);
	if(SUCCESS != rc) {
		return rc;
	} 
	...
	if((rem_len + offset) >= pClient->clientData.readBufSize) {
		qt_uart_dbg(uart1_conf.hdlr,"if the buffer is too short then the message will be dropped silently");
        ...
        {
			qt_uart_dbg(uart1_conf.hdlr,"Check buffer was correctly emptied, otherwise, return error message");
...
static IoT_Error_t _aws_iot_mqtt_internal_handle_publish(AWS_IoT_Client *pClient, Timer *pTimer) {
	...
	FUNC_ENTRY;
	qt_uart_dbg(uart1_conf.hdlr,"inside _aws_iot_mqtt_internal_handle_publish");
	topicName = NULL;
	topicNameLen = 0;
	len = 0;
	...
	qt_uart_dbg(uart1_conf.hdlr,"exiting _aws_iot_mqtt_internal_handle_publish");
	FUNC_EXIT_RC(SUCCESS);
}

IoT_Error_t aws_iot_mqtt_internal_cycle_read(AWS_IoT_Client *pClient, Timer *pTimer, uint8_t *pPacketType) {
	IoT_Error_t rc;
	qt_uart_dbg(uart1_conf.hdlr,"aws_iot_mqtt_internal_cycle_read");
#ifdef _ENABLE_THREAD_SUPPORT_
	...
	qt_uart_dbg(uart1_conf.hdlr,"calling _aws_iot_mqtt_internal_read_packet");
...
IoT_Error_t aws_iot_mqtt_internal_flushBuffers( AWS_IoT_Client *pClient ) {
	qt_uart_dbg(uart1_conf.hdlr,"inside aws_iot_mqtt_internal_flushBuffers");
...
IoT_Error_t aws_iot_mqtt_internal_wait_for_read(AWS_IoT_Client *pClient, uint8_t packetType, Timer *pTimer) {
	...
	qt_uart_dbg(uart1_conf.hdlr,"inside aws_iot_mqtt_internal_wait_for_read");
	FUNC_ENTRY;
	if(NULL == pClient || NULL == pTimer) {
		FUNC_EXIT_RC(NULL_VALUE_ERROR);
	}
	...
	do {
		...
		qt_uart_dbg(uart1_conf.hdlr," LOOP ");
	...
	qt_uart_dbg(uart1_conf.hdlr,"exiting aws_iot_mqtt_internal_wait_for_read with %d",rc);
	FUNC_EXIT_RC(rc);
}
...
IoT_Error_t aws_iot_mqtt_internal_serialize_zero(unsigned char *pTxBuf, size_t txBufLen, MessageTypes packetType,
	...
	qt_uart_dbg(uart1_conf.hdlr,"inside aws_iot_mqtt_internal_serialize_zero");
	FUNC_ENTRY;
	...
	*pSerializedLength = (uint32_t) (ptr - pTxBuf);
	...
	qt_uart_dbg(uart1_conf.hdlr,"exiting aws_iot_mqtt_internal_serialize_zero");
	FUNC_EXIT_RC(SUCCESS);
}
